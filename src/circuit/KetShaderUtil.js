 ...
 * - int out_id: The state for which we're computing an amplitude. Note that this state is relativized: when in a
 ...
 * - vec2 inp(int k): returns the amplitude of state k (as a vec2 with x=real, y=imaginary components). Note that
 *                      k is also a relativized state.
 * - int full_out_id: The non-relativized state id. Useful if you want to use the value of other qubits as an
 ...
 * - int span [if you gave an undefined span]: Two to the power of the gate height.
 * Or, if WGL2 is supported, it's just the gate height.
 *
...
const ketShader = (head, body, span=null, inputs=[]) => ({withArgs: makePseudoShaderWithInputsAndOutputAndCode(
    ...
    Outputs.vec2(),
    `
    uniform int _ketgen_step;
    ${span === null ? 'uniform int span;' : ''}
    int _ketgen_off;
    int full_out_id;

    ${body.match(/\bcmul\b/) ? 'vec2 cmul(vec2 c1, vec2 c2) { return mat2(c1.x, c1.y, -c1.y, c1.x) * c2; }' : ''}
    ${body.match(/\binp\b/) ? 'vec2 inp(int k) { return read_ketgen_ket(_ketgen_off + k'+(Config.WGL2?'*':'<<')+'_ketgen_step); }' : ''}

    ${head}

    vec2 _ketgen_output_for(int out_id, vec2 amp) {
        ${body}
    }
    ...
    vec2 outputFor(int k) {
        ...
        int relevant_out_id =
            ${Config.WGL2? `full_out_id >> _ketgen_step &
	                    (1 << ${span === null? 'span' : span}) - 1` :
            `modi(full_out_id / _ketgen_step,
		  ${span === null ? 'span' : (1<<span)})`};
        _ketgen_off = full_out_id -
                 (relevant_out_id ${Config.WGL2?'*':'<<'}_ketgen_step);

        float c = float(read_ketgen_control(full_out_id));
...
const ketShaderPermute = (head, body, span=null) => ketShader(
    head + `float _ketgen_input_for(int out_id) { ${body} }`,
...
const ketShaderPhase = (head, body, span=null) => ketShader(
    `${head}
        float _ketgen_phase_for(int out_id) {
            ${body}
        }
    `,
    `
...
function ketArgs(ctx, span=undefined, input_letters=[]) {
    let result = [
        ...
        ctx.controlsTexture,
        WglArg.int("_ketgen_step", Config.WGL2? ctx.row : 1 << ctx.row)
    ];
    if (span !== undefined) {
        result.push(WglArg.int('span', Config.WGL2? span : 1 << span));
    }
...
function ketInputGateShaderCode(letter) {
    return `
        //////// INPUT GATE ${letter} ////////
        uniform int _gen_input_default_${letter};
        uniform int _gen_input_offset_${letter};
        uniform int _gen_input_span_${letter};
        
        int read_input_${letter}() {
            return _gen_input_span_${letter} == ${Config.WGL2? -1 : 0}
                ? _gen_input_default_${letter}
                : ${Config.WGL2?
                   `full_out_id >> _gen_input_offset_${letter} &
	            (1 << _gen_input_span_${letter}) - 1` :
                   `modi(full_out_id / _gen_input_offset_${letter},
	                 _gen_input_span_${letter})`};
        }`;
}
...
function ketInputGateArgs(ctx, letter) {
    ...
    return [
        WglArg.int(`_gen_input_default_${letter}`, defaultVal),
        WglArg.int(`_gen_input_offset_${letter}`,
		     Config.WGL2? offset : 1<<offset),
        WglArg.int(`_gen_input_span_${letter}`,
		     Config.WGL2? length : length === -1 ? 0 : 1<<length),
    ];
}
